...
Search.nextResult = function(reverse) {
  ...
  Command.dataElements[this.index].scrollIntoView(false);

  switch (Command.completionResults[this.index][0]) {
    case 'chrome':
      Command.input.value =
        Command.input.value.match(/^\S+ /)[0] + Command.completionResults[this.index][1];
      break;
    case 'bookmarks':
    case 'history':
    case 'topsites':
      Command.input.value =
        Command.input.value.match(/^\S+ /)[0] + Command.completionResults[this.index][2];
      break;
    case 'tabhistory':
    ...
    case 'search':
      ...
      if (searchValue.indexOf(repl) === 0) searchValue = Utils.trim(searchValue.replace(repl, ''));
    ...
    case 'files':
      Command.input.value =
        Command.input.value.replace(/[^\/]+$/, '') + Command.completionResults[this.index][1];
      break;
    case 'settings':
