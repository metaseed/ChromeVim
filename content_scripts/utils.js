...
definePrototype(String, 'embedString', function(string) {
  if (this.indexOf('%s') === -1) return this + string;
...
var waitForESMLoad = function(callback, constructor, ...args) {
  if (window.esmLoaded) return callback.call(constructor, ...args);
  window.setTimeout(function() {
    waitForESMLoad(callback, constructor, ...args);
  }, 5);
};
...
Object.setPrototypeOf(TrieNode.prototype, {
  ...
  removeRecursiveByKey: function(keys) {
    ...
    if (node !== null) node.removeRecursive();
    return node !== null;
  },
