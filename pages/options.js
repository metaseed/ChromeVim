  ...
  RUNTIME(
    ...
    function(defaults) {
      ...
      if (res.error !== null) {
        ...
        alert('parse error on line ' + res.error.lineno + ' of config (see console for more info)');
...
Settings.editMode = function(e) {
  if (this.cssEl) {
    this.cssEl.setOption('keyMap', e.target.value === 'Vim' ? 'vim' : 'default');
  }
};
      ...
      waitForLoad(function() {
        if (Settings.initialLoad) {
          ...
          if (response.settings.localconfig && response.settings.configpath) {
            var path =
              'file://' +
              response.settings.configpath.split('~').join(response.settings.homedirectory || '~');
