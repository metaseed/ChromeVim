:Plugin 'Yggdroot/indentLine'uri-asciidoctor: http://asciidoctor.org
:icons: font
...
. add command 'displayHintForOpenInSameTab' which is useful in google search.
...
. fix gotoInput command.
. enhance the Hint switchToTop method.
. give the scrollable area hint and input hint different defaut background colors.
.
